...
export class NormalizeService {
  ...
  @Command({
    command: 'normalize:nosach-original-text',
    description: 'Normalize nosach original text',
  })
  async NormalizeNosachOriginalText(): Promise<void> {
    const normalizeLine = (l: Line) => {
      if (!l.sublines) {
        return l;
      }
      l.sublines = l.sublines.map((subline) => {
        return {
          ...subline,
          originalText: generateOriginalText(subline.nosach),
        };
      });
      return l;
    };
  ...
  async NormalizeExcerpt(): Promise<void> {
    const normalizeExcerpt = (l: MishnaExcerpt, mishna: Mishna) => {
      try {
        l.selection.fromWord = ExcerptUtils.stripCharacters(
        ...
        const fromWordTotal = ExcerptUtils.getWordsInText(
          fromLineText,
          l.selection.fromWord,
        );
        ...
        const toWordTotal = ExcerptUtils.getWordsInText(
          toLineText,
          l.selection.toWord,
        );
