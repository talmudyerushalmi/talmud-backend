...
export class NavigationService {
 ...
 async getMishnaForNavigation(tractate: string, chapter: string, mishna: string): Promise<any> {
 ...
 async getLinesForNavigation(tractate: string, chapter: string, mishna: string): Promise<string[]> {
  const mishnaDoc = await this.mishnaRepository.find(tractate, chapter, mishna);
  const lines = mishnaDoc.lines.map( l => {return l.lineNumber})
